...
/**
 * Salle de travail (amphis et tds)
 * 
 * @author ugo
 *
 */
public abstract class Workroom extends Room {
    ...
    public void action(Student s) {
        ...
        if (rand.nextInt(Game.getCourses().length + 1) == 1)
            ...
            if (estUnCours || (!estUnCours && s.peutAssissterAUnTd(course)))
                ...
                System.out.println("" + Language.VOUSSUIVI
                        + s.getNbCoursOuTdSuivis(course, true)
                        + Language.COURSET
                        + s.getNbCoursOuTdSuivis(course, false) + Language.TD2);
            }
        }
    }
    ...
    public void displayCourse(Student s) {
        ...
        System.out.println(Language.FELICITATIONS + coursOuTd
                + course.getName() + " !");
