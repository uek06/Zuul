...
public class ExamRoom extends Workroom {
	...
	public void action(Student s) {
		...
		if (rand.nextInt(Game.COURSES.length + 1) == 1)
			System.out.println(Language.PASCOURS);
		else {
			course = Game.getRandomCourse();
			System.out.println(Language.ACTUELLEMENT + " examen " + Language.DE
					+ course.getName());
			if (!(s.aSuiviTousLesCoursEtTD(course) )) {

				System.out
						.println("Vous n'avez pas suivi suffisament de cours et de TD de "
								+ course.getName());
				System.out.println("Vous avez suivi "
						+ s.getNbCoursOuTdSuivis(course, true) + " cours et "
						+ s.getNbCoursOuTdSuivis(course, false) + " TD");
				System.out.println("Vous devez en avoir suivi au minimum "
						+ course.getNbMiniCoursExam());
			} else if (!s.santeMiniPourParticiperAUnExam()) {
				System.out
						.println("Vous n'avez pas assez d'energie pour particper a l'examen");
				System.out.println("Votre energie est de " + s.getHealth());
				System.out
						.println("L'energie minimale pour passer un examen est de "
